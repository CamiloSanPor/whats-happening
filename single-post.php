<?php
  ...
  /*
    Takes information from the whats_happening database and creates a post
    Posts are created based on a Heredoc
    A querystring is used to get the eventID of the post 
  */
  function displayPost(){
    ...
    // Joins the events table with the eventtype and groups tables 
    // to get the EventType, GroupName, ContactName, and ContactEmail values
    $query = "
    SELECT EventID, EventImage, EventType, EventDate, EventTitle, EventDesc, GroupName, ContactName, ContactEmail
      FROM events
    ...
    $dateTime = date_format($dateTime, "\D\A\T\E: D d M, Y \T\I\M\E: h:i:s A");

    // index 0 is the first letter of EventDesc
    $firstLetter = $row["EventDesc"][0]; 

    // splits EventDesc into the first letter and the remaining description
    $split = explode($firstLetter, $row["EventDesc"], 2);

    // remaining text from EventDesc without the first letter
    $description = $split[1];
    ...
    <h2>Organizers: $row[GroupName]</h2>
